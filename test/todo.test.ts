...
describe('test create route', () => {
    ...
    test('Should have key record and msg when created', async () => {
        ...
        jest.spyOn(TodoInstance, 'create').mockImplementation(() =>
            mockCreateTodo()
        );

        const res = await request(app).post('/api/create').send(todo);
...
describe('test read pagination  route', () => {
    ...
    test('Should return array of todo', async () => {
        ...
        jest.spyOn(TodoInstance, 'findAll').mockImplementation(() =>
            mockReadAllTodo()
        );
